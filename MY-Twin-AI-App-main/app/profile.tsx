...
const TEXTS: Record<string, TextSet> = {
  ar: {
    ...
    energy: 'طاقة التوأم',
    recentMood: 'آخر المشاعر',
    addMood: 'تسجيل مشاعر',
    noMoodData: 'لا توجد بيانات بعد',
    howFeel: 'كيف تشعر؟',
    saveMood: 'تسجيل',
    moodRecorded: 'تم تسجيل المشاعر',
    discoveredTraits: 'الصفات المكتشفة',
  ...
  en: {
    ...
    energy: 'Twin Energy',
    recentMood: 'Recent Moods',
    addMood: 'Log Mood',
    noMoodData: 'No data yet',
    howFeel: 'How do you feel?',
    saveMood: 'Log',
    moodRecorded: 'Mood recorded',
    discoveredTraits: 'Discovered Traits',
...
export default function Profile() {
  ...
  // ✅ إصلاح: تعريف نوع texts بشكل صحيح
  const texts: TextSet = TEXTS[lang] || TEXTS.ar;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('Personality profile load error:', e);
      }
    })();
  }, [userId]);
  ...
  const fetchMoods = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      if (!cancelledRef.current) setMoodsRefreshing(false);
    }
  }, [userId]);
  ...
  const handleAddMood = async () => {
    ...
    try {
      const valence = selectedMood === 'joy' || selectedMood === 'love' 
        ? 0.8 
        : selectedMood === 'sadness' || selectedMood === 'anger' 
          ? -0.5 
          : 0.2;
      ...
      Alert.alert(t('error'), e.message || t('saveMood'));
    } finally {
      setSavingMood(false);
    }
  };
  ...
  const handleDelete = () => {
    Alert.alert(
      t('deleteAccount'),
      isAr ? 'لا يمكن التراجع.' : 'This cannot be undone.',
      [
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: bg }]}>
      ...
      >
        {/* Edit FAB */}
        <TouchableOpacity
          style={[styles.editFab, { backgroundColor: primary }]}
        ...
        <View style={[styles.userCard, { backgroundColor: card, borderColor: border }]}>
          ...
          <Text style={[styles.userName, { color: txt }]}>
            {profile.full_name || '—'}
          </Text>
          <Text style={[styles.userEmail, { color: sub }]}>
            {email || '—'}
          </Text>
        </View>
        ...
        <View style={[styles.statsGrid, { marginBottom: 14 }]}>
          <View style={[styles.statCard, { backgroundColor: card, borderColor: border }]}>
            ...
            <Text style={[styles.statLabel, { color: sub }]}>{t('bond')}</Text>
          </View>
          
          <View style={[styles.statCard, { backgroundColor: card, borderColor: border }]}>
            <TrendingUp size={18} stroke="#10B981" />
            <Text style={[styles.statValue, { color: txt }]}>
              {phaseLabels[journeyPhase] || journeyPhase}
            </Text>
            <Text style={[styles.statLabel, { color: sub }]}>{t('phase')}</Text>
          </View>
          
          <View style={[styles.statCard, { backgroundColor: card, borderColor: border }]}>
            <Brain size={18} stroke="#8B5CF6" />
            <Text style={[styles.statValue, { color: txt }]}>
              {attachmentLabels[attachmentStyle] || attachmentStyle}
            </Text>
            <Text style={[styles.statLabel, { color: sub }]}>{t('attachment')}</Text>
          </View>
          ...
          <View style={[styles.section, { backgroundColor: card, borderColor: border }]}>
            <View style={styles.sectionHeader}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <Text style={[styles.sectionTitle, { color: txt, marginBottom: 0 }]}>
                  {t('discoveredTraits')}
                </Text>
              </View>
            </View>

            {discoveredProfile.personality_traits?.length > 0 && (
            ...
            {discoveredProfile.preferences?.length > 0 && (
              <View style={{ marginBottom: 10 }}>
                ...
                <View style={styles.tagsRow}>
                  {discoveredProfile.preferences.map((pref: string, i: number) => (
                    <View key={i} style={[styles.tag, { backgroundColor: '#10B98120', borderColor: '#10B98140' }]}>
                      <Star size={12} stroke="#10B981" />
                      <Text style={[styles.tagText, { color: '#10B981' }]}>{pref}</Text>
                    </View>
                  ))}
                </View>
              </View>
            ...
            {discoveredProfile.interests?.length > 0 && (
              <View style={{ marginBottom: 10 }}>
                ...
                <View style={styles.tagsRow}>
                  {discoveredProfile.interests.map((interest: string, i: number) => (
                    <View key={i} style={[styles.tag, { backgroundColor: '#F59E0B20', borderColor: '#F59E0B40' }]}>
                      <Lightbulb size={12} stroke="#F59E0B" />
                      <Text style={[styles.tagText, { color: '#F59E0B' }]}>{interest}</Text>
                    </View>
                  ))}
                </View>
              </View>
            ...
            {discoveredProfile.beliefs?.length > 0 && (
              <View style={{ marginBottom: 10 }}>
                ...
                <View style={styles.tagsRow}>
                  {discoveredProfile.beliefs.map((belief: string, i: number) => (
                    <View key={i} style={[styles.tag, { backgroundColor: '#8B5CF620', borderColor: '#8B5CF640' }]}>
                      <Target size={12} stroke="#8B5CF6" />
                      <Text style={[styles.tagText, { color: '#8B5CF6' }]}>{belief}</Text>
                    </View>
                  ))}
                </View>
              </View>
        ...
        <View style={[styles.section, { backgroundColor: card, borderColor: border }]}>
          ...
          {editing ? (
            <>
              <TextInput
                style={[styles.input, { backgroundColor: isDark ? '#333' : '#F8F6F2', color: txt }]}
                placeholder={t('name')}
                value={name}
              ...
              <View style={styles.btnRow}>
                <TouchableOpacity style={[styles.smallBtn, { backgroundColor: primary }]} onPress={handleSave}>
                  <Text style={styles.smallBtnText}>{t('save')}</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.smallBtn, { backgroundColor: '#F0F0F0' }]} onPress={() => setEditing(false)}>
                  <Text style={[styles.smallBtnText, { color: '#666' }]}>{t('cancel')}</Text>
                </TouchableOpacity>
              </View>
            </>
          ) : (
            <>
              <View style={styles.row}>
                <User size={16} stroke={primary} />
                <Text style={[styles.value, { color: txt }]}>{profile.full_name || '—'}</Text>
              </View>
              <View style={styles.row}>
                <Mail size={16} stroke={primary} />
                <Text style={[styles.value, { color: txt }]}>{email || '—'}</Text>
              </View>
              <View style={styles.row}>
                <Phone size={16} stroke={primary} />
                <Text style={[styles.value, { color: txt }]}>{profile.phone || '—'}</Text>
              </View>
            </>
          )}
        </View>
        ...
        <View style={[styles.section, { backgroundColor: card, borderColor: border }]}>
          ...
          <View style={styles.row}>
            <Crown size={16} stroke={primary} />
            <Text style={styles.label}>{t('tier')}</Text>
            <Text style={[styles.value, { color: txt }]}>{tier}</Text>
          </View>
          <View style={styles.row}>
            <Zap size={16} stroke={primary} />
            <Text style={styles.label}>{t('messagesLeft')}</Text>
            <Text style={[styles.value, { color: txt }]}>{usage.messages || 0}</Text>
          </View>
          <View style={styles.row}>
            <MessageSquare size={16} stroke={primary} />
            <Text style={styles.label}>{t('totalMessages')}</Text>
            <Text style={[styles.value, { color: txt }]}>{totalMessages || 0}</Text>
          </View>
        </View>
        ...
        <View style={[styles.section, { backgroundColor: card, borderColor: border }]}>
          <View style={[styles.sectionHeader, isAr && { flexDirection: 'row-reverse' }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Smile size={20} stroke={primary} />
              <Text style={[styles.sectionTitle, { color: txt, marginBottom: 0 }]}>
                {t('recentMood')}
              </Text>
            </View>
            <TouchableOpacity
              style={[styles.addMoodBtn, { backgroundColor: primary }]}
          ...
          {recentMoods.length === 0 ? (
            <Text style={[styles.emptyMood, { color: sub }]}>{t('noMoodData')}</Text>
          ) : (
            recentMoods.map((entry, i) => {
              ...
              return (
                <View key={i} style={[styles.moodRow, { borderColor: border }]}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.moodLabel, { color: txt }]}>
                      {isAr ? moodInfo.label_ar : moodInfo.label_en}
                    </Text>
                    <Text style={[styles.moodTime, { color: sub }]}>
                  ...
                  <View style={[
                    styles.intensityDot,
                    {
                      ...
                      width: 12 + entry.intensity * 12,
                      height: 12 + entry.intensity * 12,
                      borderRadius: 6 + entry.intensity * 6,
                    }
                  ]} />
                </View>
              );
        ...
        >
          ...
          <Text style={styles.btnText}>{t('upgrade')}</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={[styles.btn, styles.outlineBtn, { borderColor: primary }]}
          onPress={handleLogout}
        >
          <LogOut size={16} stroke={primary} />
          <Text style={[styles.btnText, { color: primary }]}>{t('logout')}</Text>
        </TouchableOpacity>
        ...
        >
          ...
          <Text style={[styles.btnText, { color: '#EF4444' }]}>{t('deleteAccount')}</Text>
        </TouchableOpacity>
      </ScrollView>
        ...
        >
          <View style={[styles.modalContent, isDark && { backgroundColor: '#2A2A2A' }]}>
            <View style={[styles.modalHeader, isAr && { flexDirection: 'row-reverse' }]}>
              <Text style={[styles.modalTitle, { color: txt }]}>{t('howFeel')}</Text>
              <TouchableOpacity onPress={() => setShowAddMoodModal(false)}>
              ...
              {MOOD_OPTIONS.map((mood) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.moodOption,
                    {
                      borderColor: selectedMood === mood.value ? mood.color : border,
                ...
                >
                  ...
                  <Text style={[
                    ...
                    { color: selectedMood === mood.value ? mood.color : sub }
                  ]}>
                    {isAr ? mood.label_ar : mood.label_en}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <TouchableOpacity
              style={[styles.saveMoodBtn, { backgroundColor: primary, opacity: savingMood ? 0.6 : 1 }]}
            ...
            >
              {savingMood ? (
                ...
                <Text style={styles.saveMoodBtnText}>{t('saveMood')}</Text>
              )}
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>

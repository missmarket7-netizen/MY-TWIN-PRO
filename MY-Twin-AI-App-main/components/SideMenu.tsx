...
import { Home, MessageCircle, History, User, BrainCircuit, Palette, Diamond, Settings, HelpCircle, LogOut, X, PlusCircle, Gift, Sparkles, BatteryFull, BatteryMedium, BatteryLow, Heart, ArrowRight } from 'lucide-react-native';
...
export default function SideMenu({ onClose }:{ onClose:()=>void }) {
  ...
  return (
    <ScrollView style={[styles.container,{paddingTop:insets.top+20,backgroundColor:colors.bg}]} contentContainerStyle={{paddingBottom:40}} keyboardShouldPersistTaps="handled">
      ...
      {[ {icon:Home,label:t('الرئيسية','Home'),route:'/chat'}, {icon:PlusCircle,label:t('دردشة جديدة','New Chat'),onPress:startNewChat}, {icon:Heart,label:t('علاقتي','My Relationship'),route:'/relationship'}, {icon:History,label:t('سجل المحادثات','History'),route:'/history'}, {icon:User,label:t('الملف الشخصي','Profile'),route:'/profile'}, {icon:BrainCircuit,label:t('ذكريات','Memories'),route:'/memories'}, {icon:Palette,label:t('تخصيص','Customize'),route:'/customize'}, {icon:Diamond,label:t('الاشتراكات','Subscription'),route:'/subscription'}, {icon:Gift,label:t('الإحالة','Referral'),route:'/referral'}, {icon:Settings,label:t('الإعدادات','Settings'),route:'/settings'} ].map((item,idx)=>{
        const Icon = item.icon; const active = item.route ? (pathname===item.route||(item.route==='/chat'&&pathname==='/')) : false;
      ...
      <TouchableOpacity style={[styles.item,isAr&&styles.itemRTL]} onPress={()=>navigate('/help')}><HelpCircle size={20} stroke={colors.primary}/><Text style={[styles.itemLabel,{color:colors.text}]}>{t('مساعدة','Help')}</Text></TouchableOpacity>
      <TouchableOpacity style={[styles.item,isAr&&styles.itemRTL,{marginTop:20}]} onPress={()=>{ Alert.alert(t('تسجيل الخروج','Logout'),t('هل أنت متأكد؟','Are you sure?'),[ {text:t('إلغاء','Cancel'),style:'cancel'}, {text:t('خروج','Logout'),style:'destructive',onPress:async()=>{ await supabase.auth.signOut(); router.replace('/login'); }} ]); }}>
        <LogOut size={20} stroke={colors.danger}/><Text style={[styles.itemLabel,{color:colors.danger}]}>{t('تسجيل الخروج','Logout')}</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  userName:{fontSize:16,fontWeight:'700'}, bondRow:{flexDirection:'row',alignItems:'center',gap:4,marginTop:4},
  bondValue:{fontSize:12,fontWeight:'500'}, energyRow:{flexDirection:'row',alignItems:'center',gap:4,marginTop:2},
  energyValue:{fontSize:12,fontWeight:'500'}, tierText:{fontSize:12,fontWeight:'500',marginTop:2},
  ...
  itemRTL:{flexDirection:'row-reverse'}, activeItem:{backgroundColor:'#F3F0FF',borderLeftWidth:3,borderLeftColor:'#A855F7'},

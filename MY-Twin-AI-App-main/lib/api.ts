...
API.interceptors.response.use((r) => r, async (error: AxiosError) => {
  ...
  if (error.response?.status === 401 && !config._retry) { config._retry = true; try { const { data: { session } } = await supabase.auth.refreshSession(); if (session?.access_token) { _token = session.access_token; if (config.headers) config.headers['Authorization'] = `Bearer ${_token}`; return API(config); } } catch (e) {} }
  const shouldRetry = !error.response || error.response.status >= 502;
  if (shouldRetry) { config._retryCount = config._retryCount ?? 0; if (config._retryCount < 3) { config._retryCount++; await new Promise(r => setTimeout(r, Math.pow(2, config._retryCount!) * 1000)); return API(config); } }
  return Promise.reject(error);
});

export interface TwinResponse { reply: string; new_bond?: number; twin_gender?: 'male'|'female'; voice_personality?: string; emotion?: { primary: string; secondary: string; intensity: number; valence: number; arousal: number; trend?: string; riskLevel?: string }; relationship_dims?: Record<string, number>; relationship_stage?: string; journey_phase?: string; journey_day?: number; attachment_style?: string; consciousness?: { last_thought?: string; active_goals?: string[] }; memory_used?: boolean; thinking_stage?: string; dialect?: string; latency_ms?: number; energy?: number; provider?: string; }

export interface TwinRequest { message: string; twinName: string; bondLevel: number; relationshipDims: RelationshipDims; chatHistory?: Array<{ role: string; content: string }>; journeyPhase?: string; attachmentStyle?: string; twinStyle?: string; replyStyle?: string; lang?: string; image?: string; calmMode?: boolean; twinGender?: 'male'|'female'; }

function toSafeRecord(dims: RelationshipDims): Record<string, number> {
  const out: Record<string, number> = {};
  for (const [k, v] of Object.entries(dims)) { if (typeof v === 'number') out[k] = v; }
  return out;
}
...
export const askTwin = async (req: TwinRequest): Promise<TwinResponse> => {
  ...
  const payload = { message: req.message, twin_name: req.twinName||'توأمك', bond_level: req.bondLevel||0, relationship_dims: toSafeRecord(req.relationshipDims || {}), history: req.chatHistory?.slice(-10)||[], journey_phase: req.journeyPhase||'introduction', attachment_style: req.attachmentStyle||'unknown', twin_style: req.twinStyle||'supportive', reply_style: req.replyStyle||'medium', lang: req.lang||'ar', image: req.image, calm_mode: req.calmMode||false, twin_gender: g };
...
export const speakWithTwinVoice = async (text: string): Promise<void> => {
  ...
  try {
    if (store.tier && ['premium','pro','yearly'].includes(store.tier)) {
      ...
      if (response.data) { console.log('✅ صوت من الخادم'); return; }
    }
    const { speakWithGender } = require('../services/tts'); await speakWithGender(text, gender);
  } catch (error) { console.error('❌ فشل الصوت:', error); const Speech = require('expo-speech'); Speech.speak(text, { language: 'ar', pitch: gender==='male'?0.9:1.1, rate: 0.95 }); }
};

export const saveVoicePreference = async (gender: 'male'|'female', personality?: string): Promise<void> => {
  try { await API.post('/api/voice/preferences', { gender, personality: personality||'friend' }); console.log('✅ تم حفظ تفضيلات الصوت'); } catch (error) { console.error('❌ فشل:', error); }
};
